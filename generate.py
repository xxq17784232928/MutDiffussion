...
def parse_args():
    ...
    parser.add_argument("--input_text", type=str, default="This man has beard of medium length. He is in his thirties.", help="text condition")

    # paths
    parser.add_argument("--config_path", type=str, default="configs/512_codiff_mask_text.yaml", help="path to model config")

    parser.add_argument("--ckpt_path", type=str, default="pretrained/512_codiff_mask_text.ckpt", help="path to model checkpoint") 
    parser.add_argument("--save_folder", type=str, default="outputs/inference_512_codiff_mask_text", help="folder to save synthesis outputs")
...
def main():
    ...
    with torch.no_grad():
        ...
        condition = {'seg_mask': flattened_img_tensor_one_hot_transpose, 'text': [args.input_text.lower()]}
        ...
        with model.ema_scope("Plotting"):
            ...
            # DDIM sampling
            z_0_batch, intermediates = ddim_sampler.sample(S=args.ddim_steps, batch_size=args.batch_size, shape=(3, 64, 64), conditioning=condition, verbose=False, eta=1.0, log_every_t=1)

            #tensorboard的保存
            # 假设 z_0_batch 是生成的图片张量，形状为 (batch_size, 3, 64, 64)
            # 如果 z_0_batch 的范围是 [-1, 1]，我们将其转换到 [0, 1]
            ...
            for i, img_tensor in enumerate(z_0_batch1):
                ...
                # 使用 make_grid 创建图片网格 (如果需要多个图像在同一张图中)
                grid = make_grid(img_tensor)
                
                # 将图片添加到 TensorBoard，使用 add_image
                writer.add_image(f'DDIM采样得到的批量图片z_0_batch_{i}', grid, global_step=i)
